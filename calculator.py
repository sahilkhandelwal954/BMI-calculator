...
def bmi_calc():
    ...
    if(weight<0):
        resultdisplay.insert(END, "Weight cannot be negative")
    elif(height<0):
        resultdisplay.insert(END, "Height cannot be negative")
    else:
        if 0 <= bmi <= 18:
...
ew_label.config(bg="#ECE3CE",font=(30))
ew_value = StringVar()
ew = Entry(window, textvariable=ew_value)
ew.grid(row=3, column=0)
ew.config(font=(30))

resultdisplay = Text(window, height=10, width=45,bg="#B0926A")
resultdisplay.grid(row=0, column=1, rowspan=4)

calc = Button(window, text="Calculate", height=3, width=30, command=bmi_calc, bg="#D0A2F7")
calc.grid(row=4, column=1, columnspan=2)
